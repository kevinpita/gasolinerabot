...
resource "vault_policy" "gasolinerabot" {
  name = "gasolinerabot-read"

  policy = <<-EOT
    path "kv/data/apps/gasolinerabot" {
      capabilities = ["read"]
    }
...
resource "vault_kubernetes_auth_backend_role" "gasolinerabot" {
  backend                          = "kubernetes"
  role_name                        = "gasolinerabot"
  bound_service_account_names      = ["openbao-reader"]
  bound_service_account_namespaces = ["gasolinerabot"]
  audience                         = trimspace(local.openbao.audience)
  alias_name_source                = "serviceaccount_uid"
  token_policies                   = [vault_policy.gasolinerabot.name]
